...
class ConversationDao {
  ...
  Future<List<Conversation>> getAll({bool includeArchived = false}) async {
    final filtered = db.conversations
  ...
  Future<Conversation?> getById(String id) async {
    try {
      return db.conversations.firstWhere((c) => c.id == id);
    } catch (_) {
      return null;
    }
  }

  Future<void> insert(Conversation c) async {
...
class MessageDao {
  ...
  Future<void> deleteByConversation(String convId) async {
    db.messages.removeWhere((m) => m.conversationId == convId);
  }

  Future<int> count(String convId) async {
    return db.messages.where((m) => m.conversationId == convId).length;
  }
}
...
class NoteDao {
  ...
  Future<void> update(String id, String title, String content) async {
    ...
    if (i >= 0) {
      ...
      db.notes[i]['content'] = content;
      db.notes[i]['updated_at'] = DateTime.now().toIso8601String();
    }
  }

  Future<void> delete(String id) async {
    db.notes.removeWhere((n) => n['id'] == id);
  }
}
...
class PresetDao {
  ...
  Future<void> insert(ModelPreset p) async {
    db.presets.add(p);
  }

  Future<void> delete(String id) async {
    db.presets.removeWhere((p) => p.id == id);
  }
}
